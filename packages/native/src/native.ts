...
export const native = loadNative() as {
  ...
  batchParseGsdFiles: (directory: string, timeoutMs?: number) => Promise<unknown>;
  parseRoadmapFile: (content: string) => unknown;
  truncateTail: (text: string, maxBytes: number, timeoutMs?: number) => Promise<unknown>;
  truncateHead: (text: string, maxBytes: number, timeoutMs?: number) => Promise<unknown>;
  truncateOutput: (
    text: string,
    maxBytes: number,
    mode?: string,
