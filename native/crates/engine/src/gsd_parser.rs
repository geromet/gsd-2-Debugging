...
#[napi(js_name = "batchParseGsdFiles", ts_return_type = "BatchParseResult")]
pub fn batch_parse_gsd_files(
    directory: String,
    #[napi(ts_arg_type = "number | undefined")] timeout_ms: Option<u32>,
) -> task::Async<BatchParseResult> {
    // Clone directory for async task
    let directory_clone = directory.clone();
    task::blocking("batchParseGsdFiles", timeout_ms, move |ct: task::CancelToken| {
        let dir_path = Path::new(&directory_clone);
        if !dir_path.exists() {
